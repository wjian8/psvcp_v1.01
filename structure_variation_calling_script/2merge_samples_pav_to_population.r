...
h=hash()
for (file_name_depth in list.files(path=depthfile_dir,pattern=paste('*',Args[7],sep=''))){    #All the depth file was plased in the directory
    file_name=gsub(Args[7],'',file_name_depth)
    .set(h, keys=file_name ,values= read.table(file=paste(depthfile_dir,"/",file_name,Args[7],sep=''),header=F))  # read all the depth file
    }
list_chr <- NULL
for (i in keys(h)){
    list_chr <- sort(unique(c(list_chr,unique(as.character(h[[i]][,1])))))
    }
...
call_onc_chr_pav <- function(chr){  #deal with every sample with the loop
    sample_depth = h[[each_sample]]   # in the function, each_sample is just one sample
        ...
        if (nrow(one_chr_depth)==0){
        sample1_pv <- rep('A',nrow(pav_pos))
        }
        else{
