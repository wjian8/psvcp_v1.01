...
def bwa(part_sample_prefixectory): 
    for sample_prefix in part_sample_prefixectory:
        ...
        print(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))+'bwa programming finished***In threading***'+sample_prefix, file=TimeTable)
        TimeTable.flush()
        time.sleep(0.2)
        ...
        print(time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))+'picard BuildBamIndex programming finished***In threading***'+sample_prefix, file=TimeTable)
